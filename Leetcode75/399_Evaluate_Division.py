...
class Solution:
    def calcEquation(self, equations: List[List[str]], values: List[float], queries: List[List[str]]) -> List[float]:
        ...
        def backtracking(target: str, curr: dict, result: int, visited: set):
            if target in curr:
                return result * curr[target]

            keys = [key for key in graph if key not in visited]
        ...
        for i, (x, y) in enumerate(queries):
            ...
            ans[i] = backtracking(y, curr, result, visited)
